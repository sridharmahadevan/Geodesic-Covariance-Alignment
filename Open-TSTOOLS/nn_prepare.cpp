...
{		
	...
	if (nrhs < 1)
	{
		mexErrMsgTxt("nn_prepare - nearest neighbor preprocessing : data set of points (row vectors) must be given, cluster minpoints and or metric is optional");
		return;
	}
	...
	if ((metric == 0) || (!strncmp("euclidian", metric, strlen(metric)))) {
		...
		ATRIA<point_set<manhattan_distance> > searcher(points, 0, minpoints);	
		plhs[0] = nn_prepare(searcher);
		mxSetField(plhs[0], 0, "optional", mxCreateString("manhattan"));
	}
	else if (!strncmp("hamming", metric, strlen(metric))){
		point_set<hamming_distance> points(N,dim, p);	
		ATRIA<point_set<hamming_distance> > searcher(points, 0, minpoints);	
		plhs[0] = nn_prepare(searcher);
		mxSetField(plhs[0], 0, "optional", mxCreateString("hamming"));
	}
	else {
