...
void mexFunction(int nlhs, mxArray  *plhs[], int nrhs, const mxArray  *prhs[])
{
	...
	double* info = 0;		//  scaling of information dimension
	double* corr = 0;		//  scaling of correlation dimension
	
	double* hist = 0;		// counts in bins
		...
		for (long r=0; r < R; r++) { 							// we have R different partition sizes
			...
			for (long index=0; index < N; index++) {
				for (long d=0; d < dim; d++) {
					...
					const int x = long(floor(a*p[index + d*N] - b));
