...
{	
	int ref_or_direct = 1;			// ref_or_direct == 1 interpret third input argument as reference indices
											// == 0 interpret third input argument as reference vectors
	...
	if (nrhs < 4)
	{
		mexErrMsgTxt("range search : data set of points (row vectors), atria structure, reference indices or reference points \nand range must be given");
		return;
	}
	...
	const double* range = (const double *) mxGetPr(prhs[3]);		// range can be one value for all reference points
	...
	if (ref_or_direct) {		// interpret second argument as list of indices into the data set given as first argument 
		if (nrhs < 5)
		{
			mexErrMsgTxt("range_search : Data set of points (row vectors), atria structure, reference indices or reference points \nrange and past must be given must be given");
			return;
		}
...
#else
		ATRIA< point_set<euclidian_distance> > searcher(points, prhs[1]);	// this constructor used the data from the preprocessing 
#endif
