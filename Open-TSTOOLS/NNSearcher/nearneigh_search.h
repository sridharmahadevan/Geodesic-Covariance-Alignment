...
/* nearneigh_searcher<POINT_SET>::points are given and the number of nearneigh_searcher<POINT_SET>::points in the data set is small, brute force */
...
/* ATRIA might be a good choice for unevenly distributed nearneigh_searcher<POINT_SET>::points in very high dimensional spaces. */
...
template<class T>
inline void swap(T* a, const long i, const long j) { T tmp = a[i]; a[i] = a[j]; a[j] = tmp; }
...
class nearneigh_searcher : protected My_Utilities	
{
	protected :
		...
		long Nused;
#else		
		const long Nused; 			// number of nearneigh_searcher<POINT_SET>::points of the data set actually used
#endif
	...
	public :
		...
		// excl gives the number of samples from the end of nearneigh_searcher<POINT_SET>::points that should be omitted
		// from searching
...
class  Brute : public nearneigh_searcher<POINT_SET>
{
	...
	public:
		...
		// search (and count) number of nearneigh_searcher<POINT_SET>::points within distance 'radius' from the query point
		...
		// count number of nearneigh_searcher<POINT_SET>::points within distance 'radius' from the query point ("correlation sum")
		template<class ForwardIterator>
...
class ATRIA : public nearneigh_searcher<POINT_SET>
{
	...
	public:
		...
		// search for k nearest neighbors of the point query_point, excluding all nearneigh_searcher<POINT_SET>::points
...
: err(0), points(p), Nused(p.size() - excl)
#ifdef PROFILE
...
{	
	if ((nearneigh_searcher<POINT_SET>::Nused < 1) || (excl < 0))
	{
		...
		cerr << "Nused : "  << nearneigh_searcher<POINT_SET>::Nused << endl;
...
nearneigh_searcher<POINT_SET>::~nearneigh_searcher()
{
	...
	if (number_of_queries == 0)
		...
		cout << "Average percentage of points searched " 
		     << (100.0*(double)points_searched)/((double)nearneigh_searcher<POINT_SET>::Nused*number_of_queries)
...
{
	for (long j=0; j < nearneigh_searcher<POINT_SET>::Nused; j++) {
 		if ((j < first) || (j > last)) 
			test(j, query_point, nearneigh_searcher<POINT_SET>::table.highdist());	
 	}	
...
//		test(j, query_point, nearneigh_searcher<POINT_SET>::table.highdist());
//	}
//	for (j=max(last+1, first); j < nearneigh_searcher<POINT_SET>::Nused; j++) {
//		test(j, query_point, nearneigh_searcher<POINT_SET>::table.highdist());
//	}		
}
...
{
	// FIXME : make shure nearneigh_searcher<POINT_SET>::table is empty 12.Okt.1998 cmerk
	...
	return nearneigh_searcher<POINT_SET>::table.finish_search(v);	// append nearneigh_searcher<POINT_SET>::table items to v, v should be empty, afterwards nearneigh_searcher<POINT_SET>::table is empty
}
...
long Brute<POINT_SET>::search_range(vector<neighbor>& v, const double radius, ForwardIterator query_point, const long first, const long last) 
{
	...
	for (long j=0; j < nearneigh_searcher<POINT_SET>::Nused; j++)
		if ((j < first) || (j > last)) {
			const double d = nearneigh_searcher<POINT_SET>::points.distance(j, query_point);
...
long Brute<POINT_SET>::count_range(const double radius,ForwardIterator query_point, const long first, const long last) 
{
	...
	for (register long j=0; j < nearneigh_searcher<POINT_SET>::Nused; j++)
		if ((j < first) || (j > last)) {	
			if (nearneigh_searcher<POINT_SET>::points.distance(j, query_point) <= radius) count++;	
#ifdef PROFILE
			points_searched++;
#endif			
		}
...
ATRIA<POINT_SET>::ATRIA(const POINT_SET& p, const long excl, const long minpts) 
	:	nearneigh_searcher<POINT_SET>(p, excl), root(1,nearneigh_searcher<POINT_SET>::Nused-1), MINPOINTS(minpts), 
		permutation_table(new neighbor[nearneigh_searcher<POINT_SET>::Nused]), total_clusters(1),
...
pair<long, long> ATRIA<POINT_SET>::find_child_cluster_centers(const cluster* const c, neighbor* const Section, const long length)
{
	...
	if (c->Rmax == 0) { 		// if all data nearneigh_searcher<POINT_SET>::points seem to be identical
#ifdef VERBOSE		
	...
	double dist = Section[index].dist(); // nearneigh_searcher<POINT_SET>::points.distance(c->center, Section[index].index()); 
	...
	dist = nearneigh_searcher<POINT_SET>::points.distance(center_right, Section[index].index());
	Section[index].dist() = dist;
	
 	for (long i=1; i < length-1; i++) {			
 		const double d = nearneigh_searcher<POINT_SET>::points.distance(center_right, Section[i].index());
		Section[i].dist() = d;
 		if (d > dist) {
			dist = d;
...
long ATRIA<POINT_SET>::assign_points_to_centers(neighbor* const Section, const long c_length, pair<cluster*, cluster*> childs)	
{
	...
	// maximal distance fron one cluster's center to nearneigh_searcher<POINT_SET>::points belonging to this cluster
	double Rmax_left = 0;
	double Rmax_right = 0;
	...
	while(1) {
		...
		while(i+1 < j) {
			i++;
			const double dl = nearneigh_searcher<POINT_SET>::points.distance(center_left, Section[i].index());
			// reuse information instead of calculating dr = nearneigh_searcher<POINT_SET>::points.distance(center_right, Section[i].index());			
		...
		while(j-1 > i) { // either we reached the start of the array or this element was already checked by the i loop
			...
			const double dr = Section[j].dist(); // nearneigh_searcher<POINT_SET>::points.distance(center_right, Section[j].index());
			const double dl = nearneigh_searcher<POINT_SET>::points.distance(center_left, Section[j].index());
...
void ATRIA<POINT_SET>::create_tree()
{
	...
	if (nearneigh_searcher<POINT_SET>::err) return;
	...
	root.center = randindex(nearneigh_searcher<POINT_SET>::Nused);
	...
	for (k=0; k < root.center; k++) {
		const double d = nearneigh_searcher<POINT_SET>::points.distance(k, root.center);
...
void ATRIA<POINT_SET>::search(ForwardIterator query_point, const long first, const long last, const double epsilon) 
{
	...
	//mexPrintf("%d %d   -   ", first, last);
	//mexPrintf("%f\n", epsilon);
	
	const double root_dist = nearneigh_searcher<POINT_SET>::points.distance(root.center, query_point);
	...
	{	
		...
		if ((nearneigh_searcher<POINT_SET>::table.highdist() > si.dist()) && ((c->center < first) || (c->center > last)))
			nearneigh_searcher<POINT_SET>::table.insert(neighbor(c->center, si.dist()));

		if (nearneigh_searcher<POINT_SET>::table.highdist() >= (si.d_min() * (1.0 + epsilon))) {	// approximative (epsilon > 0) queries are supported			
			if (c->is_terminal())  {	
				...
				if (c->Rmax == 0.0) {
					for (long i=0; i < c->length; i++) {
						...
						if ((j < first) || (j > last)) 
							nearneigh_searcher<POINT_SET>::table.insert(neighbor(j,si.dist()));
					}	
				} else {
					for (long i=0; i < c->length; i++) { 
						const long j = Section[i].index();		
						...
						if ((j < first) || (j > last)) {
							if (nearneigh_searcher<POINT_SET>::table.highdist() > fabs(si.dist() - Section[i].dist()))
								test(j, query_point, nearneigh_searcher<POINT_SET>::table.highdist());	
						}				
					}
				}
				...
				const double dl = nearneigh_searcher<POINT_SET>::points.distance(c->left->center, query_point);
				const double dr = nearneigh_searcher<POINT_SET>::points.distance(c->right->center, query_point);
#ifdef PROFILE
				points_searched += 2;
#endif		
...
long ATRIA<POINT_SET>::search_range(vector<neighbor>& v, const double radius, ForwardIterator query_point, const long first, const long last) 
{
	...
	SearchStack.push(SearchItem(&root, nearneigh_searcher<POINT_SET>::points.distance(root.center, query_point)));
	...
	while (!SearchStack.empty()) 
	{
		...
		if (radius >= si.d_min()) {		
			...
			if (c->is_terminal())  {	// this is a terminal node
				...
				if (c->Rmax == 0.0) {		// cluster has zero radius, so all nearneigh_searcher<POINT_SET>::points inside will have the same distance to q
					if (radius >= si.dist()) {					
						for (long i=0; i < c->length; i++) {
							const long j = Section[i].index();		
...
#ifdef PARTIAL_SEARCH
							const double d = nearneigh_searcher<POINT_SET>::points.distance(j, query_point, radius);
#else
							const double d = nearneigh_searcher<POINT_SET>::points.distance(j, query_point);
#endif		
							if (d <= radius) {
			...
			if (c->is_terminal())  {	// this is a terminal terminal node
				...
				if (c->Rmax == 0.0) {		// cluster has zero radius, so all nearneigh_searcher<POINT_SET>::points inside will have the same distance to q
					...
					for (long i=0; i < c->length; i++) {
						...
						if ( ((j < first) || (j > last)) && (radius >= fabs(si.dist() - Section[i].dist())) ) {
#ifdef PARTIAL_SEARCH
							if (nearneigh_searcher<POINT_SET>::points.distance(j, query_point, radius) <= radius) 
								count++;
#else
							if (nearneigh_searcher<POINT_SET>::points.distance(j, query_point) <= radius) 
								count++;
#endif		
