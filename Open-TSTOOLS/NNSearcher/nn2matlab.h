...
ATRIA<POINT_SET>::ATRIA(const POINT_SET& p, const mxArray* inStructArr) // create an ATRIA object from data stored in struct array inStructArr
 : nearneigh_searcher<POINT_SET>(p, 0), root(0,nearneigh_searcher<POINT_SET>::Nused), MINPOINTS(0), permutation_table(0), total_clusters(1),
 total_points_in_terminal_node(0), terminal_nodes(0)
{	
	long i;
	 	
	nearneigh_searcher<POINT_SET>::err = 0;
	...
	nearneigh_searcher<POINT_SET>::Nused = (long) tmpptr[0]; 
	...
	if ((nearneigh_searcher<POINT_SET>::Nused < 2) || ( nearneigh_searcher<POINT_SET>::Nused > p.size())) {
		nearneigh_searcher<POINT_SET>::err = 1;
		return;
	}
	...
	if ((tmparr == NULL) || (!mxIsDouble(tmparr)) || (mxGetM(tmparr) != nearneigh_searcher<POINT_SET>::Nused)) {
		nearneigh_searcher<POINT_SET>::err = 1;
		return;
	}
	...
	for (i=0; i < nearneigh_searcher<POINT_SET>::Nused; i++) permutation_table[i].dist() = tmpptr[i];	
	...
	for (i=0; i < nearneigh_searcher<POINT_SET>::Nused; i++) permutation_table[i].index() = (long) tmpptr[i];	// warning : double to long conversion
	...
	while (!s.empty()) 
	{
		...
		if ((c->center < 0) || (c->center >= nearneigh_searcher<POINT_SET>::Nused)) {
			nearneigh_searcher<POINT_SET>::err = 1;
			return;
		}
		...
		if (c->is_terminal()) {
			...
			if ((c->start < 0) || (c->start >= nearneigh_searcher<POINT_SET>::Nused)) {
				nearneigh_searcher<POINT_SET>::err = 1;
				return;
			}
			
			c->length = (long) length_arr[x.index_of_node_needs_creation];
			if ((c->length < 0) || (c->length >= nearneigh_searcher<POINT_SET>::Nused)) {
				nearneigh_searcher<POINT_SET>::err = 1;
				return;
			}
		} else {
...
mxArray* ATRIA<POINT_SET>::store()	// store an ATRIA object into an MATLAB struct array
{
	...
	tmpptr[0] = nearneigh_searcher<POINT_SET>::Nused; tmpptr[1] = total_clusters; tmpptr[2] = terminal_nodes; tmpptr[3] = total_points_in_terminal_node; 
	tmpptr[4] = MINPOINTS;
	...
	tmparr = mxCreateDoubleMatrix(nearneigh_searcher<POINT_SET>::Nused, 1, mxREAL);
	tmpptr = (double *) mxGetPr(tmparr);
	
	for (i=0; i < nearneigh_searcher<POINT_SET>::Nused; i++) tmpptr[i] = permutation_table[i].dist();	
	...
	for (i=0; i < nearneigh_searcher<POINT_SET>::Nused; i++) tmpptr[i] = permutation_table[i].index();	// warning : long to double conversion
	
	mxSetField(output, 0, "indices", tmparr);
	
	// store cluster tree
	
	tree = mxCreateStructMatrix(1, 1, 6, tree_fieldnames);
