...
// DBL_MAX is needed as an upper bound for every
...
class SortedNeighborTable
{
	...
	public:
		SortedNeighborTable() : NNR(1), hd(DBL_MAX) {};
		SortedNeighborTable(const long nnr) : NNR(nnr), hd(DBL_MAX) {};
		~SortedNeighborTable() {};
			
		inline double highdist() const { return hd; };
		void insert(const neighbor& x);
		
		inline void init_search(const long nnr) { NNR = nnr; hd = DBL_MAX; }
...
class cluster
{
	...
	public:
		...
		cluster() : center(0), Rmax(DBL_MAX), start(0), length(0)  {};
		cluster(const long c) : center(c), Rmax(DBL_MAX), start(0), length(0) {};
		cluster(const long s, const long l) : center(0), Rmax(DBL_MAX), start(s), length(l)  {};
		cluster(const long s, const long l, const long c) : center(c), Rmax(DBL_MAX), start(s), length(l)  {};		
