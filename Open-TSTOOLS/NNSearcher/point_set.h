...
class point_set : public point_set_base<METRIC> {			// this implementation is for matlab matrix data
	protected:
		...
		double* const matrix_ptr; 	// points are stored as row vectors of a N by D fortran style matrix
		const METRIC Distance;		// a function object that calculates distances		
	public:
		point_set(const long n, const long d, double* mat) : point_set_base<METRIC>(n), D(d), matrix_ptr(mat), Distance() {};
		point_set(const long n, const long d, double* mat, const METRIC& metr) : point_set_base<METRIC>(n), D(d), matrix_ptr(mat), 
			Distance(metr) {};	
		...
		row_iterator point_begin(const long n) const { return row_iterator(matrix_ptr + n, point_set_base<METRIC>::N); }
		row_iterator point_end(const long n) const { return row_iterator(matrix_ptr + n + point_set_base<METRIC>::N*D, point_set_base<METRIC>::N); }	// past-the-end	

		// indices may vary between 0 and N-1, or 0 and D-1
		// vec2 must be a double vector of length D, vec1[0] ... vec1[D-1]	
		double coordinate(const long n, const long d) const { return matrix_ptr[n + point_set_base<METRIC>::N*d]; };
		...
		{
			for (register long d=0; d < D; d++) vec[d] += matrix_ptr[index + point_set_base<METRIC>::N*d];
		}
};
...
{
	public:
		spherical_point_set(const long n, double* mat) :
