...
void compute(int nlhs, mxArray  *plhs[], int nrhs, const mxArray  *prhs[], const int atria_preprocessing_given,  METRIC dummy)
{
	...
	if (mxGetM(prhs[1])*mxGetN(prhs[1]) == 3) {
		Nref_min = lmax((long) ((double *)mxGetPr(prhs[1]))[1],1);
		Nref_max = lmin((long) ((double *)mxGetPr(prhs[1]))[2], N);
	} else {
		Nref_min = 1;
		Nref_max = N;
	}	
	...
	if (nrhs > 4) 
	    bins = (long) *((double *)mxGetPr(prhs[4]));
	
	if (nrhs > 5) 
	    opt_flag = (long) *((double *)mxGetPr(prhs[5]));
	...
	long long* const total_pairs = new long long[bins];	// number of total pairs is not equal for all bins
	long long* const pairs_found = new long long[bins];	// number of pairs found within distance dist
	...
	// initialize vectors
	// pairs_found[i] counts the number of pairs resp. distances (real) smaller than dists[i]
	...
	while(((R < Nref_min) || (pairs_found[bin] < Npairs)) && (R < Nref_max)) {
		...
		if (opt_flag & (long)2) {   // count pairs twice
			first = actual-past;
			last = actual+past;
...
{	
	...
	if (atria_preprocessing_given) {
		...
		if ((metric == 0) || (!strncmp("euclidian", metric, strlen(metric)))) {
			...
			manhattan_distance dummy;
			
			if (be_verbose)
				mexPrintf("Using manhattan metric to calculated distances\n");		
			
			compute(nlhs, plhs, nrhs, prhs, 1, dummy);			
		} 
	    else if ((!strncmp("hamming", metric, strlen(metric)))) {	
			hamming_distance dummy;
			
			if (be_verbose)
				mexPrintf("Using hamming metric to calculated distances\n");		
